...
const app = express();
const cors = require("cors"); // Necessário para o frontend
...
app.post("/Filas/entrar", (req, res, next) => {
  ...
  axios
    ...
    .then((userResponse) => {
      console.log(`Usuário ${cpf_usuario} verificado. Verificando atração...`);
    ...
    .then((atracaoResponse) => {
      ...
      console.log(`Atração ${atracao_id} verificada. Colocando na fila...`);
        ...
        function (err) {
          if (err) {
            ...
            if (err.message.includes("UNIQUE constraint failed")) {
              return res.status(409).send("Erro: Usuário já está nesta fila.");
            }
          ...
          db.get("SELECT * FROM Filas WHERE id = ?", [novoId], (err, row) => {
            if (err) {
              return res
                .status(500)
                .send("Entrada na fila criada, mas falha ao buscá-la.");
            }
    ...
    .catch((error) => {
      if (error.response && error.response.status === 404) {
        ...
        if (error.config.url.includes("/Cadastro")) {
          ...
          return res.status(404).send("Recurso não encontrado via Gateway.");
        }
      } else {
        console.log("Erro ao contatar Gateway:", error.message);
        return res.status(500).send("Erro ao verificar dados via API Gateway.");
      }
    });
});
  ...
  axios
    ...
    .catch((error) => {
      ...
      if (error.response && error.response.status === 404) {
        ...
        console.log("Erro ao contatar Gateway:", error.message);
        res.status(500).send("Erro ao verificar atração via API Gateway.");
      }
    });
});
...
app.post("/Filas/sair", (req, res, next) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      if (this.changes === 0) {
        return res.status(404).send("Usuário não encontrado nesta fila.");
      }
